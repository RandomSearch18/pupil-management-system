...
class Inputs:
    ...
    def text(self, prompt, error_message="Enter some text") -> str:
        ...
        """
        stripped_input = self.valid_utf8(prompt).strip()

        if not stripped_input:
            error_incorrect_input(error_message)
            return self.text(prompt, error_message)

        return stripped_input

    def multiline(self, prompt, error_message: str = "Enter some text"):
        ...
        if full_input.strip() == "":
            error_incorrect_input(error_message)
            return self.multiline(prompt, error_message)

        return full_input

    def integer(self, prompt, error_message: str = "Enter a valid whole number") -> int:
        """Asks for a valid integer to be input"""
        raw_input = self.text(prompt, "Enter at least 1 digit")
        try:
            int_input = int(raw_input)
        except ValueError:
            error_incorrect_input(error_message)
            return self.integer(prompt, error_message)

        return int_input

    def yes_no(self, prompt, error_message='Enter "yes" or "no"') -> bool:
        """Asks for a boolean (yes or no) response"""
        YES_ANSWERS = ["yes", "y", "t", "true", "1", ":thumbs_up:"]
        NO_ANSWERS = ["no", "n", "f", "false", "0", ":thumbs_down:"]

        lowercase_input = self.text(prompt, error_message).lower()
        ...
        error_incorrect_input(error_message)
        return self.yes_no(prompt, error_message)

    def new_username(self, prompt) -> str:
    ...
    def password_to_hash(self, raw_password: str) -> str:
    ...
    def password(
        ...
        if not raw_input:
            error_incorrect_input(error_message)
            return self.password(prompt, error_message, hide_characters)

        return raw_input

    def new_password(self, prompt, hide_characters=True):
        ...
        if not hide_characters:
            return self.password_to_hash(
                self.password(prompt, error_message, hide_characters=False)
            )

        return self.password_to_hash(self.password(prompt, error_message))
    ...
    def date(self, prompt) -> datetime.date:
    ...
    def phone_number(self, prompt):
        ...
        try:
            ...
            return self.phone_number(prompt)
